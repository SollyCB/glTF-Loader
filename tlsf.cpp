...
static block_header_t *search_suitable_block(control_t *control, int *fli,
                                             int *sli) {
  int fl = *fli;
  int sl = *sli;
...
static void remove_free_block(control_t *control, block_header_t *block, int fl,
                              int sl) {
...
static void insert_free_block(control_t *control, block_header_t *block, int fl,
                              int sl) {
...
static void block_trim_free(control_t *control, block_header_t *block,
                            size_t size) {
...
static void block_trim_used(control_t *control, block_header_t *block,
                            size_t size) {
...
void tlsf_walk_pool(pool_t pool, tlsf_walker walker, void *user) {
  ...
  while (block && !block_is_last(block)) {
    pool_walker(block_to_ptr(block), block_size(block), !block_is_free(block),
                user);
  ...
  */
  ...
  if (block) {
    ...
    if (gap && gap < gap_minimum) {
      ...
      gap = tlsf_cast(size_t, tlsf_cast(tlsfptr_t, aligned) -
                                  tlsf_cast(tlsfptr_t, ptr));
    }
